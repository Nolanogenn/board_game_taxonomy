...
url = "https://boardgamegeek.com/browse/boardgamefamily"
page_text = requests.get(url).text
soup = BeautifulSoup(page_text, "html.parser")
tr_links = soup.select('tr a')
links = [(baseUrl+link["href"],link.text) for link in tr_links if len(link.text) > 0]

mechanics = [
        (link[1], get_text(link[0])) for link in tqdm(links)
        ]
with open('../data/families', 'w+') as f:
    for m in mechanics:
